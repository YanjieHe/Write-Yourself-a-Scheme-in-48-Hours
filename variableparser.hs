...
eval (List (Atom function : arguments)) = mapM (eval env) arguments >>=
  liftThrows . apply function
eval badForm = throwError $ BadSpecialForm "Unrecognized special form" badForm

apply :: String -> [LispVal] -> ThrowsError LispVal
apply function arguments = maybe (throwError $ NotFunction
                                  "Unrecognized primitive function args" function)
                           ($ arguments)
                           (lookup function primitives)
...
runRepl :: IO ()
runRepl = nullEnv >>= until_ (== "quit") (readPrompt "Lisp>>> ")
  . evalAndPrint
...
liftThrows (Left err) = TE.throwError err
...
getVar envRef var = do env <- liftIO $ readIORef envRef
                       maybe (TE.throwError $ UnboundVar
                             "Getting an unbound variable"
                             var)
                         (liftIO . readIORef)
                         (lookup var env)
